...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEmoji = function(emoji) {
        if (chatInput) {
            ...
            chatInput.focus(); // Ensure the input stays focused after adding an emoji
            console.log("Emoji added to input:", emoji); // Debug log
        } else {
            console.error("Chat input element not found. Ensure #chat-input exists in the DOM.");
        }
    };
    ...
    function setupSignOut() {
        ...
        if (signOutBtn) {
            signOutBtn.addEventListener("click", async () => {
                ...
                localStorage.removeItem('conversations'); // Clear localStorage conversations (optional, since we use DB now)
                token = "";
                user_id = "";
    
                // Clear chat history for the user from the database
                try {
                    const sender = localStorage.getItem('currentUser') || ''; // Assume you store the current username
                    await fetch('http://127.0.0.1:8000/chat/clear_user/', {
                        method: 'POST',
                        headers: {
                    ...
                    console.error('Error clearing user chat history on sign out:', error);
                }
  ...
  // Replace your existing `loadPendingFriendRequests` function with this:
    ...
    function openEditProfileModal() {
      ...
      async function initializechatFunctionality() {
        ...
        if (!chatInput) {
            console.error("Chat input element (#chat-input) not found in the DOM.");
            return; // Exit if chatInput isn’t found to prevent further errors
        }
        let sender;
        try {
            const token = localStorage.getItem('authToken');
        ...
        let chatSocket = null;
        let messageQueue = [];
        const conversations = {}; // In-memory for now, we'll fetch from DB
            ...
            chatSocket.onmessage = function(e) {
                ...
                if (data.message) {
                    ...
                    if (!exists) {
                        ...
                        // No localStorage set here; we'll rely on DB
        ...
        function sendMessage() {
            const userMessage = chatInput.value.trim(); // Use global chatInput
        ...
        function loadChatHistory() {
            ...
            if (receiver) {
                fetchChatHistoryFromDB(sender, receiver).then(messages => {
                    messages.forEach(({ text, type, time }) => {
        ...
        async function fetchChatHistoryFromDB(sender, receiver) {
            try {
                const response = await fetch(`http://127.0.0.1:8000/chat/history/?sender=${sender}&receiver=${receiver}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
        ...
        // Change: Add the addEmoji function to handle emoji selection
        ...
        // Change: Update event listeners to use updatedSendButton and updatedChatInput
    ...
    window.addEventListener('unload', async () => {
        if (receiver && sender) {
            try {
                ...
                console.error('Error clearing chat history on unload:', error);
            }
        }
    });
